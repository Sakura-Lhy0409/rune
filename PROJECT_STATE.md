...
| **当前里程碑** | ✅ **M1-13 沙箱层（输出纪律 / 资源限额 / 来源分级）**（852 测试全绿）—— 下一步见 §7 |
| **已完成里程碑** | ✅ M0 全部（…→**258 出口验收达成**）→ ✅ M1-1 ToolScheduler（282）→ ✅ M1-2 协议适配器（328）→ ✅ M1-3 波次调度（341）→ ✅ M1-4 计划与审批（379）→ ✅ M1-5 GoalEngine（405）→ ✅ M1-6 修正性重试（447）→ ✅ M1-7 上下文装配器（487）→ ✅ M1-8 工具注册表（540）→ ✅ M1-9 技能库 + 场景测试（608）→ ✅ M1-10 Workflow 引擎（657）→ ✅ M1-11 VFS 层（705）→ ✅ M1-12 自研 shell 解释器（775）→ ✅ M1-13 沙箱层（852）→ ✅ **M1-14 无 Mac 开发路径（CI + 装机 + 最小 App）** |
| **阻塞项** | 无 |
...
| C23 | ✅ VFS 层（852 测试） | `VFS.swift` | ⚠️ **两份实现跑同一套 conformance 断言** —— 一上来就抓到 4 处不一致（二进制检测/候选提示/回收站语义/末尾换行）；⚠️ **读出来必须能原样写回去**（切片曾丢掉 `endsWithNewline` → 每次往返都在改文件）；⚠️ iOS 真正咬人的是**大小写不敏感**（`README.md` 在 iOS 上就是 `readme.md`，T34）；⚠️ **`==` 在"要比字节形式"的地方是陷阱**（两处静默失效，T35）；⚠️ 快照在手机上很贵，`clonefile` 待 macOS |
| C24 | ✅ 自研 shell 解释器（852 测试） | `Shell.swift` | ⚠️ **"不支持什么"比"支持什么"更重要** —— 每条不支持的语法都要给**可执行的替代**（否则模型只是换个写法再试，来回烧 token）；⚠️ **裸 `$VAR` 必须被拒**（放过去会得到静默的错误答案：命令收到字面量 `$HOME`，然后以看不懂的方式失败；双引号里也一样，T36）；⚠️ **管道中间环节的 stdout 不算可见输出**（否则模型会看到 `hello` 与 `HELLO`，以为跑了两遍，T37）；⚠️ 默认**失败即停且 `;` 也不例外**，但 `||` 后面那条必须照跑 |
| C25 | ✅ 沙箱层（852 测试） | `Sandbox.swift` | ⚠️ **输出纪律只有一份实现**（内联 ≤8KB / 制品 ≤2MB / 更大则**拒绝**并指向"改做法"）；⚠️ 制品说明必须给全 **5W**（多大/多少行/里面有什么/去哪读/怎么读）；⚠️ **超限判定顺序：内存与指令数排在墙钟前面**（墙钟超时往往是它们的结果，先报超时会把真因藏起来）；⚠️ **沙箱内存额度 = 物理内存的 1/4**（3/4 会让 iOS jetsam 杀掉**整个 App**，T38）；⚠️ 来源分级：模型生成 → WASM，做不到则"降级 + 强制确认"而**不是放行**（T39） |
| C26 | ✅ 事件日志与投影（852 测试） | `EventLog.swift` | ⚠️ **唯一真相源**：哈希链 + 定期锚点（锚点必须存在 Keychain 等 **Agent 够不到**的地方，否则是自证清白）；⚠️ 三种损坏**分别**报出来（内容被改 / 衔接断了 / 记录被删）—— 用户看到这三句话的感受完全不同；⚠️ **增量投影 == 全量重放**（这条一破，缓存视图与真相就分叉，而分叉后没有东西能告诉你哪个对）；⚠️ **冷启动分诊**：链坏了就什么都不自动做；有外部副作用的必须问用户；僵尸(>24h)只标记中断不自动续跑 |
| C27 | ✅ **无 Mac 开发与验证路径**（`docs/16`） | `.github/workflows/` · `Tools/ci.sh` · `Tools/check_ci.py` · `Apps/Rune/` | ⭐ GH Actions 的 macOS 运行器负责编译测试，Windows 的 Sideloadly 负责签名装机；⚠️ Linux 那条**把「零 Apple 依赖」变成 CI 断言**；⚠️ 免费 Apple ID 签名只有 7 天（$99/年去掉限制并开 TestFlight）；⚠️ **交互式调试永久做不到** —— 工作方式变成「写测试 → 推 CI → 看报告」 |

---

## 6. 下一步

### ⚠️ 现状：Windows 上能做的纯逻辑基本做完（852 测试）
...
`RuneKernel` 有 **35 个源文件、852 项测试**，覆盖：值类型与协议、补丁引擎、检索、网关、
...
> **用户没有 macOS 机器。** 而这件事**有解**：**GitHub Actions 的 macOS 运行器负责编译与测试，
> Windows 上的 Sideloadly 负责签名与装机。** 唯一真正做不到的是**交互式**调试
> （断点 / Instruments / 手动点模拟器）→ 工作方式变成「写测试 → 推 CI → 看报告」。
> ⚠️ 免费 Apple ID 签名只有 **7 天**；$99/年去掉限制并开 TestFlight。
> 已就位：`.github/workflows/{kernel,ios}.yml`、`Tools/{ci.sh,check_ci.py,lint_quotes.py}`、`Apps/Rune/`。
> ⚠️ **第一次 CI 失败是正常的** —— `Package.swift` 从未被真正的 SwiftPM 解析过（E1）。
> 细节见 [`docs/16`](docs/16-无Mac开发与验证路径.md)。

**路线 A 的实现顺序**（CI 就绪之后）：
`RuneStore`（GRDB + 事件落盘）→ `RuneNet`（URLSession + SSE + 出口代理）→
`RuneBench`（VFS 落地 + bookmark + CPython 垫片）→ `RuneTools`（86 个工具实现）→
`RuneCore`（接上真实 IO）→ `RuneUI`（docs/08 那套交互）。
每一层都要**带上测试**，因为验证只能走 CI。

**路线 B（Windows 上纯粹逻辑性的收尾）**：C18–C25 已完成。剩下三件较小的：
检索融合（RRF）、MCP 编解码、cassette 回放夹具。**价值远低于路线 A，不要用它们填时间。**
...
├─ Tools/  rune.ps1(本机唯一入口) · ci.sh(与 CI 同一套检查) · check_ci.py · lint_quotes.py
├─ docs/
...
└─ Packages/
   ├─ RuneKernel/            ✅ 零依赖核心（35 源文件 / 852 测试全绿；Sources 35 个 .swift、Tests 23 个）
   └─ Rune{Net,Store,VM,Bench,Gateway,Context,Core,Tools,MCP,UI}/   ⬜ 骨架（含实现清单）
```

**`RuneKernel` 源文件一览**：值类型与安全 `JSONValue` `SHA256` `Trust` `Content` `Tool` `Capability` `Errors`；
编辑/检索/执行 `TextPatch` `GlobMatcher` `IgnoreRules` `GrepEngine` `VFS` `Shell` `Sandbox`；
