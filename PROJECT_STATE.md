...
| 项 | 值 |
|---|---|
| **更新日期** | 2026-09-18（C48 Git 引擎地基） |
| **当前阶段** | ▶️ **真实模型端到端跑通（C46）+ 开始攻执行/平台能力（Git 引擎 C47/C48）** —— 1120 包测试全绿。整项目未完工 |
| **当前里程碑** | ✅ **M0 出口标准·云端版（C46）** —— 真实模型（`gpt-5.5`）自主完成「读 → 改 → 复读确认」，途中经审批门与崩溃恢复校验。⚠️ 此前「C45 云推理被账户拒绝」是**选题错误造成的假结论** |
| **已完成里程碑** | ✅ M0 全部（…→**258**）→ ✅ M1-1…M1-15（→877）→ ✅ M1-16…M1-23（→1032）→ ✅ **M1-24 模拟器冒烟** → ✅ **M1-25 交接 macOS** → ✅ **M1-26 RuneStore 跑绿（C39）** → ✅ **M1-27 RuneNet（C41）** → ✅ **M2-2 真实运行时接线（C45）** → ✅ **M0 云端出口验收（C46）** → 🔨 **M3-1 Git 引擎（C47/C48/C49 地基已绿，3/4；差 packfile）** |
...
**包结构**：Kernel **1075** / Net **24** / Store **14** / Core **15** / Tools **7** / UI **6** 测试，共 **1141**。Core 已有真实运行时，Tools 有 PDF/OCR/CSV，Store schema v2 含原子检查点；其余 5 包仍是骨架。11 包可构建。
---
...
## 6. 下一步
**主线现状**：C46 打通真实云端推理后，正在攻**平台能力**。**Git 引擎已开工（C47/C48/C49，地基 3/4）**：iOS 无 fork/exec ⇒ 没有系统 git，Rune 必须**自己读懂 .git**。已完成纯 Swift zlib inflate、SHA-1/对象寻址、松散对象读取 + tree/commit/引用解析。⚠️ **下一步必须是 packfile + delta 解析** —— `git clone` 下来的对象几乎全在 pack 里，只支持松散对象不足以处理真实历史，它必须排在写路径之前。之后是执行宿主（CPython/WASM/JSC）、MCP、记忆检索/L3 压缩与真机验收，**不可把本轮当成全项目完成**。
...
③ 工具共 **87 个注册 / 18 个已实现**；剩余最大一块是 **Git（14 个，C47/C48 进行中）**、执行宿主（CPython/JSC/WASM）、网络（URLSession 已就位，工具壳未接）、iOS 原生（相册/日历/定位…）；
...
`RuneKernel`（44 源文件 / 1075 测试）覆盖：值类型与协议、补丁、检索、渠道网关、成本熔断、结构化压缩、出站构建与体检、模型调用客户端、策略引擎、Turn 循环与崩溃恢复、波次调度、计划/审批/目标、修正性重试与协议不变式、上下文装配、86 工具契约、技能库、Workflow、VFS、自研 shell、沙箱层 —— **并有端到端场景测试证明它们拼得起来**。
...
> ⚠️ **E1–E8 全部是 Windows 时期的坑，在 macOS 上不复现**（保留是为了以后有人在 Windows 上接手时不必重新踩）。C39 另记两条 macOS 环境坑，见 7.2 的 **T57 / T58**。
