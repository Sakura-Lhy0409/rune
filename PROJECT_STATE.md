...
| **当前里程碑** | M1-1：工具调度器（并行分桶 + 冲突检测 + 预算） |
| **上一个完成的里程碑** | ✅ **M0-6：M0 出口验收达成** —— "模型自主修好失败单测 + 任意切断点恢复后状态完全一致"（258 测试全绿） |
| **已完成里程碑** | ✅ M0-1（85）→ M0-2（122）→ M0-3（172）→ M0-4（212）→ M0-5（249）→ **M0-6 出口验收（258）** |
| **阻塞项** | 无 |
...
| 项 | 状态 | 备注 |
|---|---|---|
| M1-1 工具调度器 | 🚧 即将开始 | 并行分桶 + 冲突检测 + 预算（本机可测） |
| M1-2 协议适配器 | ⬜ 未开始 | 请求序列化 + SSE/NDJSON 解析（本机可测） |
...
### 立即（M1-1，纯逻辑、本机可测）
1. **`ToolScheduler`**：一轮里的多个工具调用如何并行
   - 分桶：`parallelSafe`（只读）并行 / `serialPerPath`（同路径写）串行 / `exclusive` 独占
   - **冲突检测**：桶内任意两个写操作的路径前缀重叠 → 强制降级为串行（**不依赖模型自觉**）
   - 并发上限（`min(6, 性能核数×2)`，手机上要保守）
   - 审批项批量提交（手机上一次弹一个审批是灾难）
2. **修正性重试策略**：把 `ToolError.isSelfCorrectable` 接进循环（同一次调用最多重试 2 次），
   并在事件里区分"模型自己改好的"与"放弃的"。

### 随后（M1-2，纯逻辑、本机可测，**M3 的地基**）
3. **协议适配器**（`RuneGateway` 的核心，纯逻辑可测）：
...
### 随后（M1-3 / M1-4）
4. **`PlanEngine` + `ApprovalBroker`**：计划生成、偏离检测（`Plan.requiresUserConfirmation` 已就位）、
   审批请求的批量与"记住选择"。
5. **`GoalEngine`**：跨轮推进 + 阻塞纪律（`Goal` 类型已就位，缺驱动器）。
