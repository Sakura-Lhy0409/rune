...
| **当前里程碑** | M1-5：`GoalEngine`（跨轮推进 + 阻塞纪律） |
| **上一个完成的里程碑** | ✅ **M1-1：ToolScheduler**（并行分桶 + 冲突检测 + 预算）（282 测试全绿） |
| **已完成里程碑** | ✅ M0 全部（…→**258 出口验收达成**）→ ✅ M1-1 ToolScheduler（282）→ ✅ M1-2 协议适配器（328）→ ✅ M1-3 波次调度（341）→ ✅ **M1-4 计划与审批（379）** |
| **阻塞项** | 无 |
...
| 项 | 状态 | 备注 |
|---|---|---|
| M1-5 Goal 引擎 | 🚧 即将开始 | 跨轮推进 + 阻塞纪律（本机可测） |
