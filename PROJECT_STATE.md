...
| **当前里程碑** | M1-4：`PlanEngine` + `ApprovalBroker`（计划生成与审批批量） |
| **上一个完成的里程碑** | ✅ **M1-1：ToolScheduler**（并行分桶 + 冲突检测 + 预算）（282 测试全绿） |
| **已完成里程碑** | ✅ M0 全部（…→**258 出口验收达成**）→ ✅ M1-1 ToolScheduler（282）→ ✅ M1-2 协议适配器（328）→ ✅ **M1-3 波次调度（341）** |
| **阻塞项** | 无 |
...
| 项 | 状态 | 备注 |
|---|---|---|
| M1-4 计划与审批 | 🚧 即将开始 | PlanEngine + ApprovalBroker（本机可测） |
| M1-4 计划与审批 | ⬜ 未开始 | PlanEngine + ApprovalBroker |
| M1-5 Goal 引擎 | ⬜ 未开始 | 跨轮推进 + 阻塞纪律 |
