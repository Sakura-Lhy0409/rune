...
| **已完成里程碑** | ✅ M0 全部（…→**258**）→ ✅ M1-1…M1-15（→877）→ ✅ M1-16…M1-23（→1032）→ ✅ **M1-24 模拟器冒烟** → ✅ **M1-25 交接 macOS** → ✅ **M1-26 RuneStore 跑绿（C39）** → ✅ **M1-27 RuneNet（C41）** → ✅ **M2-2 真实运行时接线（C45）** → ✅ **M0 云端出口验收（C46）** → 🔨 **M3-1 Git 引擎（C47/C48 地基已绿，2/4）** |
...
**包结构**：Kernel **1054** / Net **24** / Store **14** / Core **15** / Tools **7** / UI **6** 测试，共 **1120**。Core 已有真实运行时，Tools 有 PDF/OCR/CSV，Store schema v2 含原子检查点；其余 5 包仍是骨架。11 包可构建。
---
...
## 6. 下一步
**主线现状**：C46 打通真实云端推理后，正在攻**平台能力**。**Git 引擎已开工（C47/C48，地基 2/4）**：iOS 不允许 fork/exec，所以没有系统 git 可用，Rune 必须**自己读懂 .git** —— 已完成纯 Swift 的 zlib inflate 与 SHA-1/对象寻址，下一步是松散对象读取 + tree/commit 解析 → `git_log`/`git_show`。之后是执行宿主（CPython/WASM/JSC）、MCP、记忆检索/L3 压缩与真机验收，**不可把本轮当成全项目完成**。
...
`RuneKernel`（43 源文件 / 1054 测试）覆盖：值类型与协议、补丁、检索、渠道网关、成本熔断、结构化压缩、出站构建与体检、模型调用客户端、策略引擎、Turn 循环与崩溃恢复、波次调度、计划/审批/目标、修正性重试与协议不变式、上下文装配、86 工具契约、技能库、Workflow、VFS、自研 shell、沙箱层 —— **并有端到端场景测试证明它们拼得起来**。
