...
⚠️ **实现时的实测修正：中文的 L0 成本约为上表的 2 倍。**

上表的「~15 token」是**按英文估的**（英文 1 token ≈ 4 字符）。而中文是 **1 字符 ≈ 1 token**，
同样的信息量在中文里要 30–40 token。用 15 去卡，结果是所有中文技能条目全部「超标」——
那不是条目写坏了，是尺子错了。

换算之后的账：**40 条 × ~35 token ≈ 1400 token**。
在 200k 窗口里是 0.7%，在 32k 窗口里是 4.4%，都不成问题；
**但在端侧模型那 4096 token 的窗口里是 34%** —— 所以端侧反射模式**不带技能目录**。
这就是为什么这个预算必须是一个显式校准过的数字，而不是「越小越好」的口号。

⚠️ **L0 的 token 预算必须把标题行与提示行一起算进去**，否则它就不是硬预算：
只累加条目成本的话，「预算 300」会实际产出 306，超出的部分会悄悄挤掉别的区块的额度。

### 7.4 技能不能给自己提权

技能包里的 `policy.toml` 是**申请**，不是**授权**：

* 技能永远不能申请人类专属区（策略文件、信任档、凭据、审计日志）——声明即拒绝注册；
* 权限必须在**加载正文之前**就位。不能「先加载、遇到越权再拒绝」：
  正文本身就会引导模型去申请那些权限，那已经是一次提权尝试了；
* 权限覆盖判定按**作用域包含**（授了 `/workspace/src` 就覆盖 `/workspace/src/utils`），
  域名按**点边界后缀**（授了 `example.com` 覆盖 `api.example.com`，但不覆盖 `evil-example.com`）。

⚠️ **L1 正文永远不能自动加载。** 一旦「顺手把正文也带上」，渐进式披露就退化成「全塞」——
而它退化之后**不会有任何报错**，只是账单变贵、模型变慢、关键信息被淹没。
所以 `SkillRegistry` 从 API 设计上就**不提供**「取全部正文」这种方便方法。
...
真正的执行（起子代理、并发、调模型）由运行时做。换来三件事：

* **完全确定性** —— 同样的输入必然产出同样的决策，可以逐步骤断言，不需要等并发；
* **天然可恢复** —— 每个步骤的状态都在 `WorkflowRun` 里，被系统杀掉接着跑就行；
* **并发策略可替换** —— 手机上限是 3，将来放宽到 8 不用改引擎。

反过来，如果让引擎自己 spawn 并发任务，这三条会同时失去 —— 而它们正是移动端最需要的三条。

**② `pipeline` 的无屏障语义靠「深度降序」实现。**

设想 17 个目标、每个要过"检查 → 修复 → 验证"：

* 按**书写顺序**调度 → 先把 17 个检查全跑完再开始修复，那实际上是**三级屏障**，
  与 §8.1 的 pipeline 语义直接矛盾；
* 按**深度降序**调度 → 某个 item 的检查一完成，它的修复步骤就优先于其他 item 还没开始的检查 ——
  流水线真的流动起来。深度相同则按书写顺序、按 id 兜底 → 排序完全确定。

**③ 上游返回 `null` 会整条链短路，且与「失败」严格区分。**

`prev == null` → 本步骤 `skipped`（结果也是 `null`），并沿链传下去。
跳过与失败在三处必须区分：状态（`skipped` / `failed`）、汇总（`null` / `{error}`）、
进度看板（"跳过 3" / "失败 1"）。用户要能看懂"这是判断不需要做"还是"这是没做成"。

**④ `addSteps` 必须先把运行"重新打开"。**
...
### 8.5 ⚠️ 脚本静态安全检查**不是**安全边界

`WorkflowScriptGuard` 会扫描脚本里的 `fetch` / `require` / `eval` / `setTimeout` /
`globalThis` / `__proto__` 之类的标识符，但它**不是**安全控制。

> 真正的边界是 **JSC 宿主根本不暴露这些全局**：没有 `fetch`、没有文件 API、没有定时器。
> 脚本就算写出 `fetch(...)`，也只是"调用了一个 undefined"，什么都不会发生。
...
所以：**命中 ≠ 攻击，只是"值得提醒"；不命中 ≠ 安全。** 这句话必须写进实现者的脑子里，
